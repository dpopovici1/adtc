(function() {
	...
	tinymce.create('tinymce.plugins.Addshortcodes', {
		init : function(ed, url) {
			...
			//AddBoxs
			ed.addButton('AddAlert', {
				title : 'Add Alert',
				cmd : 'tinyBoxes',
				image : url + '/images/alert.png'
			});
			ed.addCommand('tinyBoxes', function() {
				ed.windowManager.open({file : url + '/ui.php?page=alert',width : 600 ,	height : 450 ,	inline : 1});
			});
			...
			ed.addCommand('tinymaps', function() {
				ed.windowManager.open({file : url + '/ui.php?page=googlemap',width : 600 ,	height : 180 ,	inline : 1});
			});
	...
	tinymce.PluginManager.add('asalahShortCodes', tinymce.plugins.Addshortcodes);	
